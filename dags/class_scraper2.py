...
def connection_test() :
    ...
    print(f'*Checking connection to this url')
    time.sleep(2)
    response=requests.get('https://www.kompas.com/tag/jawa+barat')
    #response=requests.get('https://www.kompas.com/tag/jawa+barat')
    status=response.status_code
    if status != 200 :
      print('*There is problem with the connection')
    else :
      #print('*Connected to Kompas Website')
      status='Hey, it is conneected!'
    
    return status
    #push to xcom with custom id
    #.xcom_push(key="connection_status", value=any_serializable_value)
...
class scraper:
  ...
  def run_scrapper(self) :
    ...
    """
    #check connection and create condition to make this method run only if connection is available
    if self.connection_test() == 200 :
